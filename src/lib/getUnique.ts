...
export const getDefaultOptions = (data: Tx[]) => {
  ...
  for (const tx of data) {
    if (!person.has(tx.person)) {
      person.add(tx.person);
    }
    if (!account.has(tx.konto)) {
      account.add(tx.konto);
    }
    if (!category.has(tx.budgetgrupp)) {
      category.add(tx.budgetgrupp);
    }
  }
...
const reorder = (arr: string[]): string[] => [
  ...
  ...arr.filter((v) => v !== "inkomst" && v !== "övrigt").toSorted((a, b) => a.localeCompare(b)),
