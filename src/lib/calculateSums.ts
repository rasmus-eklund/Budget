...
const calculateSums = ({
  ...
  for (const p of person) {
    total[p] = category.reduce((acc, category) => (acc += sums[category]![p]!), 0);
  ...
  total.total = Object.keys(total).reduce((acc, person) => (acc += total[person]!), 0);
  spending.total = Object.keys(spending).reduce((acc, person) => (acc += spending[person]!), 0);
